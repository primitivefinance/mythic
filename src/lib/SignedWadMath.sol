...
library SignedWadMathLib {
    /// @dev Will not revert on overflow, only use where overflow is not possible.
    function toWadUnsafe(uint256 x) internal pure returns (int256 r) {
    ...
    function fromDaysWadUnsafe(int256 x) internal pure returns (uint256 r) {
        /// @solidity memory-safe-assembly
        assembly {
    ...
    function wadDiv(int256 x, int256 y) internal pure returns (int256 r) {
        ...
        assembly {
            ...
            if iszero(
                and(iszero(iszero(y)), eq(sdiv(r, 1000000000000000000), x))
            ) { revert(0, 0) }
    ...
    function wadPow(int256 x, int256 y) internal pure returns (int256) {
    ...
    function wadExp(int256 x) internal pure returns (int256 r) {
        unchecked {
            ...
            if (x >= 135_305_999_368_893_231_589) revert("EXP_OVERFLOW");
    ...
    function unsafeDiv(int256 x, int256 y) internal pure returns (int256 r) {
        /// @solidity memory-safe-assembly
        assembly {
            // Divide x by y.
            r := sdiv(x, y)
