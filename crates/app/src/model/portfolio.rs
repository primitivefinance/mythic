...
impl DataModel<AlloyAddress, AlloyU256> {
    ...
    pub async fn strategy(&self, client: Arc<Client>) -> Result<LogNormal<Client>> {
        let converted_address = to_ethers_address(self.strategy_address);
    ...
    pub async fn solver(&self, client: Arc<Client>) -> Result<LogNormalSolver<Client>> {
        let converted_address = to_ethers_address(self.solver_address);
    ...
    async fn fetch_block_number(&self, client: Arc<Client>) -> Result<u64> {
        let block_number = client.get_block_number().await.unwrap_or(0.into());
    ...
    async fn update_internal_price(&mut self, client: Arc<Client>) -> Result<()> {
        ...
        self.internal_spot_price = internal_price;
    ...
    fn update_external_price_series(&mut self) -> Result<()> {
        let default = (0u64, self.external_spot_price);
        let last_element = self.external_spot_price_series.last().unwrap_or(&default);
        if last_element.0 >= self.latest_block {
            return Ok(());
        }
    ...
    fn update_user_quote_value_series(&mut self) -> Result<()> {
        let default = (0u64, self.user_quote_balance);
        let last_element = self.user_quote_value_series.last().unwrap_or(&default);
        if last_element.0 >= self.latest_block {
            return Ok(());
        }
    ...
    fn update_protocol_asset_value_series(&mut self) -> Result<()> {
        let default = (0u64, self.protocol_asset_balance);
        let last_element = self.protocol_asset_value_series.last().unwrap_or(&default);
        if last_element.0 >= self.latest_block {
            return Ok(());
        }
        let asset_value = self
    ...
    fn update_protocol_quote_value_series(&mut self) -> Result<()> {
        let default = (0u64, self.protocol_quote_balance);
        let last_element = self.protocol_quote_value_series.last().unwrap_or(&default);
        if last_element.0 >= self.latest_block {
            return Ok(());
        }
        let quote_value = self
...
mod tests {
    ...
    async fn setup() -> anyhow::Result<Arc<SignerMiddleware<Provider<Ws>, LocalWallet>>> {
    ...
    async fn test_fetch_balance_of() -> anyhow::Result<()> {
        ...
        let model = DataModel::<AlloyAddress, AlloyU256>::new();
    ...
    async fn test_update_token_balances() -> anyhow::Result<()> {
        ...
        // Now we can fetch the balance of the wallet.
        let mut model = DataModel::<AlloyAddress, AlloyU256>::new();
