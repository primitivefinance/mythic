...
contract SG3M is IG3M, IStrategy {
    ...
    UD60x18 public startWeightX;

    /// @dev Target weight of token X.
    UD60x18 public targetWeightX;

    uint256 public startWeightUpdate;

    uint256 public endWeightUpdate;
     ...
     * @param startWeightX_ Weight of token X, expressed in WAD (note that `weightY`
     * will be computed as `1 WAD - weightX`).
     */
    constructor(
        address tokenX_,
        address tokenY_,
        UD60x18 startWeightX_,
        UD60x18 targetWeightX_,
        uint256 endWeightUpdate_,
        uint256 swapFee_
    ) {
        ...
        require(startWeightX_ >= MIN_WEIGHT, "Start weight X too low");
        require(startWeightX_ <= MAX_WEIGHT, "Start weight X too high");

        require(targetWeightX_ >= MIN_WEIGHT, "Target weight X too low");
        require(targetWeightX_ <= MAX_WEIGHT, "Target weight X too high");

        startWeightX = startWeightX_;
        targetWeightX = targetWeightX_;
        startWeightUpdate = block.timestamp;
        endWeightUpdate = endWeightUpdate_;
    ...
    function setWeightX(
        ...
        uint256 newEndWeightUpdate
    ) public onlyAdmin {
        require(newTargetWeightX >= MIN_WEIGHT, "Weight X too low");
        require(newTargetWeightX <= MAX_WEIGHT, "Weight X too high");
        require(newEndWeightUpdate > block.timestamp, "Update end pasted");

        startWeightX = weightX();
        startWeightUpdate = block.timestamp;

        targetWeightX = newTargetWeightX;
        endWeightUpdate = newEndWeightUpdate;
    ...
    function weightX() public view returns (UD60x18) {
        if (block.timestamp >= endWeightUpdate) {
